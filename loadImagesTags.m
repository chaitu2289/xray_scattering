function [ images,tags,m,n ] = loadImagesTags()
    ...
    for k=3:length(Files)
        ...
        image =  im2double(imread(strcat(imageDir,FileName)));
        [m,n] = size(image);
        ...
        if k ~= length(Files)
            ...
            images{1}{i,2} = image;%reshape(image,m,n,1);
            i = i + 1;
        else
            images{2}{1,1} = FileName;
            images{2}{1,2} = reshape(image,m,n,1);
        end
    end
